...
├ 1. fullpage.js가 제공해주는 해당 페이지 포커스시 애니메이션 작동하는 동작 사용<ㅠㄱ>
└ 2. 화면을 계속 바꾸어가며 키캡들의 위치를 수동 조작하였음.
* 지원 페이지 아코디언
├ 이미 열려있는 아코디언을 다시 눌렀을경우 닫히게, 아코디언들을 display : block, none이 아닌 height로 애니메이션 동작 가능하게 만들려고 하였음.
├ 1. isOpen 변수를 만들고 if문으로 열려있는지 확인하였고 열려있을 경우 닫히게 만들었다.
